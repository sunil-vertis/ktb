...
2. Create a config file named `codegen.cjs` in the root of your project (see the repo’s `codegen.cjs`):

Use the repository’s `codegen.cjs`: it wires `OPTIMIZELY_API_URL` / `OPTIMIZELY_SINGLE_KEY` and, when `OPTIMIZELY_GRAPH_TLS_INSECURE=1`, uses `scripts/graphql-codegen-fetch.cjs` so schema introspection works behind TLS-inspecting proxies.
...
{
  "scripts": {
    "gen-types": "node -r ./scripts/node-tls-prelude.cjs ./node_modules/@graphql-codegen/cli/cjs/bin.js --config ./codegen.cjs"
  }
}
```
